...
- 대시보드는 기록 파일을 **브라우저 안에서만** 읽는다. 업로드 경로를 만들지
  않는다.
- **반응시간은 순환 중에 고른 것만 남긴다.** 머무름 연타는 커서를 기다린 것이 아니라
  `reactionMs`가 비어 있다. 값으로 거르면 `Number(null)`이 0이 되어 0ms 표본으로 섞인다.
- **쉬고 있던 시간은 놓침이 아니다.** 두 바퀴가 넘도록 누르지 않으면 `steps`를 되돌린다.
  이 규칙이 없으면 앱을 켜 둔 시간이 그대로 놓침으로 잡힌다.
...
하드웨어는 별도 트랙으로 제작 중이다. 스위치는 **아두이노 우노를 거쳐 시리얼로**
`P`(눌림) / `R`(뗌)을 보낸다. 우노는 USB HID가 물리적으로 불가능해서 전역 단축키로는
영영 받을 수 없다 — 2026-08-23에 실물로 확인했다(PRD 7절).

`switch.rs`가 시리얼을 읽고 `input.rs`의 `GestureDetector`에 넣는다. 전역 단축키
경로는 하드웨어 없이 검증할 때만 쓴다.

**펌웨어의 프로토콜을 바꾸면 `switch.rs`와 PRD 7절을 함께 고친다.** 특히 `FLASH`/`OFF`는
LED를 켜는 유일한 통로라서(접근성 원칙 4, PRD F7) 없애면 기능이 사라진다.
