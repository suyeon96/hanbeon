...
- 칸을 2열 격자로 배치한다. 창 기본 크기 360×240, 기본 위치는 주 모니터 우하단.
  (칸 위에 드래그 손잡이와 남은 시간 막대가, 아래에 상태 줄이 한 줄씩 붙는다)
- **칸 수는 앱에 따라 4~7칸으로 달라지고, 창 높이가 함께 바뀐다**(F11). 행이
  하나 늘 때마다 68px. 창은 **좌상단을 고정하고 아래로 자란다** — 그래야 앞
  4칸의 화면상 자리가 유지된다. 화면 밖으로 나가면 그만큼만 위로 올린다.
...
- 지금 앞에 있는 앱에 맞춰 **앞 4칸 뒤에 최대 3칸**을 더 붙인다.
- **앞 4칸의 순서와 자리는 어떤 앱에서도 바뀌지 않는다.** 사용자는 자리로
  동작을 기억한다.
